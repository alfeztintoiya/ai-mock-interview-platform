...
// Client should not instantiate Gemini SDK; use server route instead
...
// Removed direct client-side SDK usage
...
`;
    ...
    const respGen = await fetch("/api/generate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ prompt: InputPrompt }),
    });
    const {questions,error} = await respGen.json();
    if (error || !Array.isArray(questions)) {
      console.error("Generation error:", error);
      setLoading(false);
      return;
    }
    // const MockJsonResp = (genJson.text || "")
    //   .replace("```json", "")
    //   .replace("```", "")
    //   .trim();
